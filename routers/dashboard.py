...
async def _save(request: Request, data: dict):
    from routers.auth import save_session, get_session_id
    sid = get_session_id(request)
    if sid:
        await save_session(sid, data)

def _check_auth_sync(session: dict):
    if not session.get("user"):
        return RedirectResponse("/")
    return None

def _check_guild_sync(session: dict, guild_id: str):
    if not any(g["id"] == guild_id for g in session.get("admin_guilds", [])):
...
async def select_guild(request: Request, guild_id: str):
    ...
    await _save(request, session)

    return RedirectResponse(f"/dashboard/{guild_id}/overview", status_code=302)

@router.get("/{guild_id}/overview")
async def overview(request: Request, guild_id: str):
    session = await _session(request)
    r = _check_auth_sync(session) or _check_guild_sync(session, guild_id)
    if r: return r
